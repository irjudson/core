...
var attach = function(server) {
    ...
    this.pub = redis.createClient(core.config.redis_server.port, core.config.redis_server.host);
    this.sub = redis.createClient(core.config.redis_server.port, core.config.redis_server.host);
    this.client = redis.createClient(core.config.redis_server.port, core.config.redis_server.host);
        ...
        function(callback) {
            receive(subscription, function(err, item, ref) {
                ...
                if (socket.subscriptions[subscription.clientId] === undefined) {
                    ...
                    if (item) {
                        core.log.debug('subscription service:  new message from subscription: ' + subscription.clientId + ' with name: ' + subscription.name + ' of type: ' + subscription.type + ": " + JSON.stringify(item));
